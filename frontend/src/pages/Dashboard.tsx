...
export const Dashboard = () => {
  ...
  const githubRepositories = repositories.filter((repo) => repo.source === 'github');
  const repositoryCount = githubRepoCount > 0 ? githubRepoCount : githubRepositories.length;
  ...
  const stats = [
    ...
    {
      label: 'Risk Score',
      value: aiAnalysis?.analyses?.[0]?.risk_score?.toFixed(2) || impactAnalysis?.risk_score?.toFixed(2) || '0.0',
      icon: AlertTriangle,
      color: 'text-red-500',
      bg: 'bg-red-950/20',
              ...
              repositories.map((repo, i) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 className="font-bold text-white group-hover:text-red-500 transition-colors">{getDisplayName(repo)}</h4>
